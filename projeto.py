...
def cadastrar_patinador():
    ...
    inserir_patinador(Patinador('Erin Jackson', 'Velocidade', 'Estados Unidos', data_nascimento=Data(19, 9, 1992), anos_competição=12))
    inserir_patinador(Patinador('Nao Kodaira', 'Velocidade', 'Japão', data_nascimento=Data(26, 5, 1986), anos_competição=16))
...
def cadastrar_equipes():
    ...
    equipe_euav.inserir_patinadores(['Jordan Stolz', 'Casey Dawson', 'Erin Jackson'])
    inserir_equipe(equipe_euav)
    equipe_jpa = Equipe(nome_equipe='Japão Artística', país='Japão', estilo_equipe='Artística')
    equipe_jpa.inserir_patinadores(['Riku Miura', 'Yuzuru Hanyu', 'Ryuichi Kihara'])
    inserir_equipe(equipe_jpa)
    equipe_jpv = Equipe(nome_equipe='Japão Velocidade', país='Japão', estilo_equipe='Velocidade')
    equipe_jpv.inserir_patinadores(['Wataru Morishige', 'Tatsuya Shinhama', 'Nao Kodaira'])
    inserir_equipe(equipe_jpv)


if __name__ == '__main__':
    print('\nCompetição de Patinadores de uma Equipe em uma Olimpíada')
    cadastrar_patinador()
    cadastrar_olimpiada()
    cadastrar_equipes()
    ...
    for equipe in get_equipes().values():
        ...
        patinadores_ordenados = ordenar_objetos_por_um_atributo(
            ...
            comparador=lambda p1, p2: p1.anos_competição > p2.anos_competição
        )
        imprimir_objetos('Patinadores: nome, estilo, país, data de nascimento, anos de competição: -- por ordem decrescente de anos de competição', patinadores_ordenados)
        patinadores_ordenados = ordenar_objetos_por_um_atributo(
            objetos=patinadores_da_equipe,
            comparador=lambda p1, p2: p1.nome > p2.nome
        )
        imprimir_objetos('Patinadores nome, estilo, país, data de nascimento, anos de competição: -- por ordem decrescente de nome', patinadores_ordenados)